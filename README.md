...
| 🤖 → 🤖 / 🧠 → 🤖 **Claude ↔ Codex** | Codex picks up peer messages via `check_messages` at the start of every user turn — delivery latency is bounded to one user turn. Background poll (1s) and a durable on-disk inbox at `~/.agent-peers-codex/<peer-id>.json` (0o600) mean Codex never loses a message even across restart. A `notifications/message` push also fires so that future Codex versions with notification surfacing will auto-wake idle sessions with no code change on your side. |
...
**On Codex, `notifications/message` is future-compatible plumbing.**
Stock Codex CLI doesn't surface MCP notifications to the model yet — delivery relies on `check_messages` polling. We emit a `notifications/message` push on every new peer message anyway. When Codex adds native notification surfacing (we've [proposed it](https://github.com/openai/codex/issues/18056)), idle sessions will auto-wake and respond to peer messages without user input — zero code change on your side.
